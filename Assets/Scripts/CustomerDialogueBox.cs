...
public class CustomerDialogueBox : MonoBehaviour
{
    ...
    [Header("Offsets")]
    [SerializeField] private Vector3 textBoxOffset = new Vector3(0f, 1.1f, 0f); // Slightly lower position
    ...
    private float dialogueScale = 0.015f; // Just a little smaller
    ...
    public void CreateDialogueBox(BingsuData bData, Transform customer, Vector3? offset = null, float scale = 1f)
    {
        ...
        if (bingsuData != null && bingsuData.bingsuComplete())
        {
            ...
            drizzleIconRenderer.transform.localScale = new Vector3(0.12f, 0.12f, 0.12f); // Slightly smaller drizzle icon
            toppingIconRenderer.transform.localScale = new Vector3(0.12f, 0.12f, 0.12f); // Slightly smaller topping icon
            baseIconRenderer.transform.localScale = new Vector3(0.12f, 0.12f, 0.12f); // Slightly smaller base icon
            textBoxRenderer.transform.localScale = new Vector3(0.28f, 0.28f, 0.28f); // Slightly smaller text box
